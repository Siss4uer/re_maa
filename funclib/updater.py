...
from master.src.re_dev.funclib.log import logger
from master.src.re_dev.funclib import tools
...
class Updater:
	...
	def update(self):
		...
		if not latest_version:  # latest_version为False代表获取失败
			self.logger.info("获取版本信息失败")
		elif current_version == latest_version:  # 通过比较二者是否一致判断是否需要更新（摆烂
			self.logger.info("当前为最新版本，无需更新")
		else:
			self.logger.info(f"检测到最新版本:{latest_version}，正在更新")
			# 开始更新逻辑
			...
			if not url_list:
				...
				self.logger.info("未找到适用于当前系统的更新包")
				# 直接结束
				return
			# 将路径和文件名拼合成绝对路径
			...
			for retry_frequency in range(max_retry):
				self.logger.info("开始下载" + (f"，第{retry_frequency}次尝试" if retry_frequency > 1 else ""))
				# 选择最低延迟的镜像
				url = tools.choose_url(url_list)
				self.logger.info(f"下载地址:{url}")
				try:

					stdout, stderr = tools.aria2c_download(url)
